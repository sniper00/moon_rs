...
local function hotfix_(root, proto_map, short_source)
	...
	local function collect_all_uv(funcs, source)
		...
		for _, v in pairs(funcs) do
			if type(v) == "function" then
				collect_uv(v, all_upvalues, source)
			end
		end
		if not all_upvalues["_ENV"] then
			all_upvalues["_ENV"] = { func = collect_uv, index = 1 }
		end
		return all_upvalues
	end

	local function update_func(all_upvalues, f)
		if exclude[f] then
			return
		end
		exclude[f] = true
		...
		while true do
			...
			if type(value) == "function" then
				update_func(all_upvalues, value)
			else
				local old_uv = assert(all_upvalues[name])
