...
/// # Safety
///
/// The `from_*` methods return references that borrow from the Lua stack.
/// The pointer returned by `lua_tolstring` / `luaL_checklstring` is only
/// guaranteed valid while the value stays on the stack and no GC cycle runs.
/// **Do not store the returned reference beyond the current Lua C callback.**
///
/// Prefer the standalone `unsafe` functions (`lua_check_lstring`,
/// `lua_to_lstring`, `lua_opt_lstring`) which make the safety contract
/// explicit at each call site.
impl LuaStack for &[u8] {
    fn from_checked(state: LuaState, index: i32) -> &'static [u8] {
        unsafe { lua_check_lstring(state, index) }
    }

    fn from_unchecked(state: LuaState, index: i32) -> &'static [u8] {
        unsafe { lua_to_lstring(state, index) }
    }

    fn from_opt(state: LuaState, index: i32) -> Option<&'static [u8]> {
        unsafe { lua_opt_lstring(state, index) }
    }
...
/// # Safety
///
/// Same caveats as `impl LuaStack for &[u8]` — the returned `&str` borrows
/// from the Lua stack and must not outlive the current C callback scope.
///
/// Prefer `lua_check_str`, `lua_to_str`, `lua_opt_str`.
impl LuaStack for &str {
    fn from_checked(state: LuaState, index: i32) -> &'static str {
        unsafe { lua_check_str(state, index) }
    }

    fn from_unchecked(state: LuaState, index: i32) -> &'static str {
        unsafe { lua_to_str(state, index) }
    }

    fn from_opt(state: LuaState, index: i32) -> Option<&'static str> {
        unsafe { lua_opt_str(state, index) }
    }
...
// ---------------------------------------------------------------------------
// Explicit unsafe accessors for Lua stack string values
//
// The returned references borrow memory owned by the Lua GC.  They are only
// valid while the value remains on the Lua stack and no GC cycle runs.
// Callers must NOT store these references beyond the current C callback scope.
// ---------------------------------------------------------------------------

/// Returns a checked `&[u8]` from the Lua stack (calls `luaL_checklstring`).
///
/// # Safety
/// The returned slice borrows from the Lua stack. It must not be stored beyond
/// the current Lua C function call.
pub unsafe fn lua_check_lstring(state: LuaState, index: i32) -> &'static [u8] { unsafe {
    let mut len = 0;
    let ptr = ffi::luaL_checklstring(state.as_ptr(), index, &mut len);
    std::slice::from_raw_parts(ptr as *const u8, len)
}}

/// Returns an unchecked `&[u8]` from the Lua stack (calls `lua_tolstring`).
///
/// # Safety
/// See [`lua_check_lstring`].
pub unsafe fn lua_to_lstring(state: LuaState, index: i32) -> &'static [u8] { unsafe {
    let mut len = 0;
    let ptr = ffi::lua_tolstring(state.as_ptr(), index, &mut len);
    std::slice::from_raw_parts(ptr as *const u8, len)
}}

/// Returns an optional `&[u8]` from the Lua stack if the value is a string.
///
/// # Safety
/// See [`lua_check_lstring`].
pub unsafe fn lua_opt_lstring(state: LuaState, index: i32) -> Option<&'static [u8]> { unsafe {
...
/// Returns a checked `&str` from the Lua stack. Validates UTF-8;
/// raises a Lua error if the string contains invalid UTF-8 bytes.
///
/// # Safety
/// See [`lua_check_lstring`].
pub unsafe fn lua_check_str(state: LuaState, index: i32) -> &'static str { unsafe {
    let bytes = lua_check_lstring(state, index);
    match std::str::from_utf8(bytes) {
        Ok(s) => s,
        Err(e) => lua_error(
            state,
            format!(
                "bad argument #{index} (valid UTF-8 string expected, invalid byte at position {pos})",
                pos = e.valid_up_to()
            ),
        ),
    }
...
/// Returns an unchecked `&str` from the Lua stack. Returns `""` if the
/// string is not valid UTF-8.
///
/// # Safety
/// See [`lua_check_lstring`].
pub unsafe fn lua_to_str(state: LuaState, index: i32) -> &'static str { unsafe {
    std::str::from_utf8(lua_to_lstring(state, index)).unwrap_or_default()
}}

/// Returns an optional `&str` from the Lua stack if the value is a string.
/// Returns `None` if the value is not a string or is not valid UTF-8.
///
/// # Safety
/// See [`lua_check_lstring`].
pub unsafe fn lua_opt_str(state: LuaState, index: i32) -> Option<&'static str> { unsafe {
    lua_opt_lstring(state, index).and_then(|s| std::str::from_utf8(s).ok())
}}

/// Converts the value at `index` to a string via `luaL_tolstring` (pushes +1).
///
/// # Safety
/// The returned slice borrows from the Lua stack. It must not be stored beyond
/// the current Lua C function call. The caller must pop the pushed value.
pub unsafe fn lua_as_slice(state: LuaState, index: i32) -> &'static [u8] { unsafe {
    let mut len = 0;
...
pub fn lua_array_size(state: LuaState, idx: i32) -> usize {
    unsafe {
        ...
        if first_key <= 0 {
            ...
            // lua_next returning 0 already popped the key; no extra pop needed.
        }
