...
unsafe extern "C" fn allocator(
    ...
    unsafe {
        ...
        if nsize == 0 {
            if !ptr.is_null() {
                let layout = Layout::from_size_align_unchecked(osize, moon_lua::SYS_MIN_ALIGN);
        ...
        if ptr.is_null() {
            // Allocate new memory
            let new_layout = match Layout::from_size_align(nsize, moon_lua::SYS_MIN_ALIGN) {
                Ok(layout) => layout,
        ...
        // Reallocate memory
        let old_layout = Layout::from_size_align_unchecked(osize, moon_lua::SYS_MIN_ALIGN);
