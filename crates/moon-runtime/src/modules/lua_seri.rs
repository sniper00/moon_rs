...
fn unpack_one(state: LuaState, br: &mut ReadBlock, depth: usize) {
    let type_ = br.read_byte();
    push_value(state, br, type_ & 0x7, type_ >> 3, depth);
}

fn push_value(state: LuaState, br: &mut ReadBlock, type_: u8, cookie: u8, depth: usize) {
    match type_ {
        TYPE_NIL => {
...
fn unpack_table(state: LuaState, br: &mut ReadBlock, mut array_size: usize, depth: usize) {
    if depth > MAX_DEPTH {
        laux::lua_push(state, "serialize unpack: too deep");
...
extern "C-unwind" fn peek_one(state: LuaState) -> c_int {
    unsafe {
        ...
        push_value(state, br, type_ & 0x7, type_ >> 3, 0);
