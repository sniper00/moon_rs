...
moon.async(function()
    ...
    -- JSON table as parameter (table is auto-encoded)
    res = db:query(
        "INSERT INTO sqlx_test (col_integer,col_jsonb) VALUES ($1,$2)",
        100, {items = {1,2,3}, nested = {a = "b"}}
    )
    assert_nil("param json table insert ok", res and res.kind)

    res = db:query("SELECT col_jsonb FROM sqlx_test WHERE col_integer=$1;", 100)
    assert_match("param jsonb from table", res[1].col_jsonb, '"items"')

    -- Explicit JSON via sqlx.json() with a table
    res = db:query(
        "INSERT INTO sqlx_test (col_integer,col_jsonb) VALUES ($1,$2)",
        101, sqlx.json({color = "red", size = 42})
    )
    assert_nil("sqlx.json table insert ok", res and res.kind)

    res = db:query("SELECT col_jsonb FROM sqlx_test WHERE col_integer=$1;", 101)
    assert_match("sqlx.json table result", res[1].col_jsonb, '"color"')

    -- Explicit JSON via sqlx.json() with a string
    res = db:query(
        "INSERT INTO sqlx_test (col_integer,col_json) VALUES ($1,$2)",
        102, sqlx.json('{"explicit":true}')
    )
    assert_nil("sqlx.json string insert ok", res and res.kind)

    res = db:query("SELECT col_json FROM sqlx_test WHERE col_integer=$1;", 102)
    assert_match("sqlx.json string result", res[1].col_json, '"explicit"')

    -- String starting with { is NOT auto-detected as JSON anymore (treated as text)
    res = db:query(
        "INSERT INTO sqlx_test (col_integer,col_varchar) VALUES ($1,$2)",
        103, "{not json}"
    )
    assert_nil("brace string as text ok", res and res.kind)

    res = db:query("SELECT col_varchar FROM sqlx_test WHERE col_integer=$1;", 103)
    assert_eq("brace string stored as text", res[1].col_varchar, "{not json}")
